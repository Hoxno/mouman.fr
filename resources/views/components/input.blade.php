<div>
    ...
    <div @class(['mt-2', 'relative rounded-md shadow-sm' => $errors->has($name)])>
        @if ($type === 'textarea')
        <textarea class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full @error($name) is-invalid @enderror" type="{{ $type }}" id="{{ $name }}" name="{{ $name }}" cols="5" rows="5">{{ old($name, $value) }}</textarea>
        @else
