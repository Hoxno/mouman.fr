...
<section class="py-8 md:pt-32 md:pb-12" id="skills">
    <h2 class="relative text-secondary mt-4 mb-8 text-center after:content-[''] after:absolute after:w-16 after:h-1 after:bg-secondary after:top-12 after:right-0 after:left-0 after:m-auto md:mb-12 md:after:w-20 md:after:top-12">
        Mes compétances
    </h2>
    ...
    @foreach ($skills as $skill)
        ...
        <div class="flex justify-between items-center relative font-normal py-2 px-4 mb-8 rounded-lg shadow-4xl bg-[#fff]">
            <div class="flex items-center">
                <i class="fa-brands fa-{{ $skill->title }} mr-2 text-html5"></i>
                <span class="font-bold">{{ $skill->title }}</span>
            </div>
            <div>
                <span class="font-bold">{{ $skill->level }}%</span>
            </div>
            <div class="absolute left-0 bottom-0 h-1 rounded-lg z-10 w-10/12 bg-{{ $skill->title }}"></div>
        </div>
        @endif
    @endforeach
    </div>
